...
test('collapses duplicated /tw path segments', () => {
  const input = 'Tracked link https://learnstockmarket.online/tw/tw/strategies';
  const output = enforcePromoTrackingPath(input);
  assert.match(output, /https:\/\/learnstockmarket\.online\/tw\/strategies\b/);
  assert.doesNotMatch(output, /\/tw\/tw\//);
});

...
Start the scheduler (runs the tweet job at 8:00, 10:00, 12:00, 14:00, 16:00, 18:00 Central Time on the hour, plus **7:45 PM** CT — `America/Chicago` in `src/scheduler.ts`):
...
You can also keep the process running with `npm run schedule` in a terminal, or use system cron to run `npm run tweet` 7 times per weekday: six runs on the hour at 8:00–18:00 CT as above, plus **19:45** CT (same as `src/scheduler.ts`).

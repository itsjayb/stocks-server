 ...
 * │ 14,16,18 │  + 7:45 PM (hourly on the hour otherwise)           │
 * └──────────┴──────────────────────────────────────────────────────┘
 *
...
if (RUN_TWEET_JOB) {
  ...
  cron.schedule('45 19 * * 1-5', () => runJob('tweet', 'tweet-job.ts'), { timezone: TZ });
  console.log('[scheduler] Tweet job: 8 AM, 10, 12, 2, 4, 6 PM on the hour; 7:45 PM CT (Mon–Fri).');
} else {

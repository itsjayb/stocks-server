 ...
 * │ 14,16,18,│  8 AM – 8 PM CT on the hour                        │
 * │ 20       │                                                      │
 * └──────────┴──────────────────────────────────────────────────────┘
 *
...
// ── Tweet job (opt-in) ──
if (RUN_TWEET_JOB) {
  const TWEET_SCHEDULE = '0 8,10,12,14,16,18,20 * * 1-5';
  cron.schedule(TWEET_SCHEDULE, () => runJob('tweet', 'tweet-job.ts'), { timezone: TZ });
  console.log('[scheduler] Tweet job: 8 AM, 10, 12, 2, 4, 6, 8 PM CT on the hour (Mon–Fri).');
} else {

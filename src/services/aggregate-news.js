 ...
 * Kept in sync with aggregate-news.ts (tweet types: news, pattern, strategy; promo URLs use /tw/).
 */
...
export const TWEET_TYPES = ['news', 'pattern', 'strategy'];

export function isValidTweetType(t) {
  return TWEET_TYPES.includes(t);
}

/**
 * @param {Date} [now]
 */
export function pickTweetType(now = new Date()) {
  const d = now;
